package de.andrena.refactoringlikecompilers;
...
import de.andrena.refactoringlikecompilers.UglyClass;

class UglyClassTest {
	private UglyClass frankMustermann = new UglyClass("m", "Frank", "Mustermann");
	private UglyClass erikaMusterfrau = new UglyClass("f", "Erika", "Musterfrau");
	...
	private TypeSafeDiagnosingMatcher<UglyClass> matchesName(String name) {
		return new TypeSafeDiagnosingMatcher<UglyClass>() {
			...
			@Override
			protected boolean matchesSafely(UglyClass item, Description mismatchDescription) {
				return item.isTheSameAs(name);
			}
		};
	}

	private TypeSafeDiagnosingMatcher<UglyClass> doesNotMatchName(String name) {
		return new TypeSafeDiagnosingMatcher<UglyClass>() {
